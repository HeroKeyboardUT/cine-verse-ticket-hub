...
class ReportController {
  async getStatisticReport(req, res) {
    try {
      ...
      res
        .status(500)
        .json({ message: "Error fetching statistic report", error });
    }
  }
  async getMonthlyRevenueReport(req, res) {
    try {
      ...
      res
        .status(500)
        .json({ message: "Error fetching monthly revenue report", error });
    }
  }
  async getDailyRevenueReport(req, res) {
    try {
      ...
      res
        .status(500)
        .json({ message: "Error fetching daily revenue report", error });
    }
  }
  async getMovieRevenueReport(req, res) {
    try {
      ...
      res
        .status(500)
        .json({ message: "Error fetching movie revenue report", error });
    }
  }
  async getTopCustomerReport(req, res) {
    try {
      ...
      res
        .status(500)
        .json({ message: "Error fetching top customer report", error });
    }
  }
  async getLocationRevenueReport(req, res) {
    try {
      const locationId = req.params.id;
      const report = await reportModel.getLocationRevenueReport(locationId);
      res.status(200).json(report);
    } catch (error) {
      res
        .status(500)
        .json({ message: "Error fetching location revenue report", error });
    }
  }
}
